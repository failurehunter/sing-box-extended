...
| `with_grpc_lite`                   | :material-close:️    | Use the lite gRPC engine for all V2Ray gRPC transports even when `with_grpc` is enabled. Allows custom `user_agent` and keeps the client behavior identical to the default (non-`with_grpc`) build.                                                                                                                            |
